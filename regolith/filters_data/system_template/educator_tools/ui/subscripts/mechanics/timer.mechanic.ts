...
export class Timer {
	...
	private worldData: WorldData; // Add this property
	 ...
	 * @param worldData - The worldData instance.
	 * @param time - The current time of the timer timer.
	...
	constructor(
		...
		// Initialize the worldData property
	...
	private updateTimer() {
		...
		if (this.time <= 0) {
			this.paused = true;
			this.worldData.clearTimer(); // Clear the timer in WorldData
			this.despawnTimer();
		} else {
			this.timerEntityHealthComponent.setCurrentValue(this.time);
			this.timerEntity.triggerEvent(
